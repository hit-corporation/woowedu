...
(async $ => {
    ...
    $('#tbl_materi > tbody').on('click', '.btn.view_video', e => {
        ...
        document.getElementById('video-file').src = 'http://203.128.89.90/web/uploads/materi/' + data.materi_file;
        document.getElementById('note').innerHTML = null;
        document.getElementById('note').innerHTML = data.note;
        $('#modal-video').modal('show');
    });
 
    $('#tbl_materi > tbody').on('click', '.btn.upload_soal', e => {
				let target = e.target;
			  const data = table.row($(target).parents('tr')).data();
				document.getElementById('soal_mapel').value = data.subject_id;
				document.getElementById('soal_materi').value = data.materi_id;
    ...
    $('#modal-import').on('hidden.bs.modal', function(e) {
        ...
        var progress = document.getElementById('import-progress-1');
    ...
    /**
		...
		*/
		...
		let impSoalForm = document.forms['import-soal'];
		let impSoalZip = impSoalForm['zip-upload-soal'],
				impMapel = impSoalForm['soal_mapel'],
				impMateri = impSoalForm['soal_materi'],
				impSoalFile = impSoalForm['file-upload-soal'];

		let importSoalZip = null,
				importSoalFile = null; 
		impSoalFile.addEventListener('change', e => {
				e.preventDefault();
				let lbl = document.querySelector('.xl_label');
				lbl.innerHTML = e.target.files[0].name;
				importSoalFile = e.target.files[0];
		});
		
		impSoalZip.addEventListener('change', e => {
				e.preventDefault();
				let lbl = document.querySelector('.zip_label');
				lbl.innerHTML = e.target.files[0].name;
				importSoalZip = e.target.files[0];
		});
			
    impSoalForm.addEventListener('submit', e => {
			e.preventDefault();
			let fdata = new FormData();
			fdata.append('subject_id', impMapel.value);			
			fdata.append('materi_id', impMateri.value);			
			fdata.append('upload-soal-zip', importSoalZip);
			fdata.append('upload-soal-file', importSoalFile);
			// upload 
			$.ajax({
					xhr: function() {
						 var xhr = new window.XMLHttpRequest();
						 var prog = document.getElementById('import-progress-soal');
						 xhr.upload.addEventListener('progress', e1 => {
								 if(e1.lengthComputable) {
										 prog.removeAttribute('hidden');
										 var completed = (e1.loaded === e1.total) ? 90 : Math.round((e1.loaded / e1.total) * 100);
					...
					url: base_url + 'api/materi/importsoal',
					type: 'POST',
					data: fdata,
					contentType: false,
					processData: false,
		...
		$('#modal-import-soal').on('hidden.bs.modal', function(e) {
		 impSoalForm.reset();
		 impSoalForm.querySelector('.custom-file-label').innerHTML = 'Pilih File';
		 var progress = document.getElementById('import-progress-soal');
		 if(progress.hasAttribute('style')) {
				 progress.getElementsByClassName('progress-bar')[0].removeAttribute('style');
				 progress.getElementsByClassName('progress-bar')[0].setAttribute('aria-valuenow', 0);
				 progress.getElementsByClassName('progress-bar')[0].innerHTML = '';
